...
static int my_hex_to_int(char c)
{
  if (c >= '0' && c <= '9')
    return c - '0';
  if (c >= 'A' && c <= 'F')
...
static uintptr_t pattern_scan(const uintptr_t base, const uintptr_t base_size, const char *pattern)
{
...
uintptr_t pattern_scan_offset(const uintptr_t base, const uintptr_t base_size, const char *pattern, const size_t ret_offset)
{
...
static uintptr_t kernel_chunk_scan(const char *pattern, const size_t offset_to_pattern)
{
  uint8_t buf[0x4000];
  ssize_t imgsize;
  size_t len;
  ...
  for (size_t i = 0; i < imgsize; i += sizeof(buf))
  {
    ...
    uintptr_t res = pattern_scan_offset(pBuf, sizeof(buf), pattern, offset_to_pattern);
    if(res)
    {
...
int main(void) {
  ...
  case 0x6710000:
  case 0x6720000:
    enable_ptrace_patch1 = "48 b8 36 10 00 00 7e 02 00 00 ? ? ? ? ? ? ? ? ? ? ? ? ? 0f 84 ? ? ? ?";
    offset_to_nop = 23;
    break;
  case 0x7000000:
  case 0x7020000:
  ...
  case 0x12500000:
  case 0x12520000:
        enable_ptrace_patch1 = "48 b8 36 10 00 00 7e 02 00 00 4c 0f a3 e0 0f 83 ? ? ? ? 85 db 0f 84 ? ? ? ?";
        offset_to_nop = 22;
    break;
  ...
  if (file_check_r) {
    ...
    const uintptr_t patternRes = kernel_chunk_scan(enable_ptrace_patch1, offset_to_nop);
    if (patternRes) {
